...
def addHand(cards):  #Sum up the hand, used for decision making
    hand = sum(cards)
    aces = cards.count(11)
    while hand > 21 and aces > 0: #conver aces to 1s if they would cause a bust
        hand -= 10
        aces -= 1
    return hand
...
def trainQ(rounds=50000, alpha=0.1, gamma=.99, epsStart=1.0, epsEnd=.05):
    Q = {}
...
def evalPolicy(Q, episodes=10000):
    ...
    for _ in range(episodes):
        ...
        while True:
            ...
            state = nextState
    n = episodes
    print(f"greedy policy -> wins:{wins/n:.3f} losses:{losses/n:.3f} pushes:{pushes/n:.3f}")

def evalRandom(episodes=10000):
    wins=losses=pushes=0
    for _ in range(episodes):
        r = playRound()  # your 50/50 Hit/Stand loop
        if r>0: wins+=1
        elif r<0: losses+=1
        else: pushes+=1
    n = episodes
    print(f"random policy -> wins:{wins/n:.3f} losses:{losses/n:.3f} pushes:{pushes/n:.3f}")


if __name__ == "__main__":
    
    random.seed(1000)
    Q = trainQ(rounds=500000)
